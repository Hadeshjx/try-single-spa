...
// 我们处理应用加载的逻辑是在最前面
window.addEventListener('hashchange', urlRerote)
window.addEventListener('popstate', urlRerote)
...
// 浏览器路由，浏览器路由是h5 api的 如果切换 是不会触发popstate
...
function patchedUpdateState(updateState,methodName){
    return function(){
        ...
        if(urlBefore!==urlAfter){
            // 重新加载应用 传入事件源
            urlRerote(new PopStateEvent('popstate'))
        }
    }
}

window.history.pushState = patchedUpdateState(window.history.pushState,'pushState')
window.history.replaceState = patchedUpdateState(window.history.pushState,'replaceState')

// 用户可能还会绑定自己的路由事件 如：vue
